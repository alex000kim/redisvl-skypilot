...
async def lifespan(app: FastAPI):
    ...
    index = SearchIndex.from_yaml("config/redis_schema_papers.yaml", redis_client=redis_client)
    try:
...
async def search(request: SearchRequest):
    ...
    query = VectorQuery(
        ...
        return_fields=["title", "abstract", "authors", "venue", "year", "n_citation"],
    ...
    return SearchResponse(results=papers, total=len(papers), time_ms=round(time_ms, 2))
