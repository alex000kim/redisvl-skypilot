...
def main():
    ...
    df = pd.read_csv(args.input_file, encoding='utf-8', encoding_errors='replace')
    total_records = len(df)
    
    if args.start_idx >= total_records:
        logger.warning(f"Start index {args.start_idx} >= total records {total_records}")
        return
    
    end_idx = min(args.end_idx, total_records)
    logger.info(f"Processing records {args.start_idx}-{end_idx} of {total_records}")
    
    redis_client = create_redis_client(
        args.redis_host, args.redis_port, args.redis_user, args.redis_password
    )
    ...
    with tqdm(total=len(df_partition), desc=f"Records {args.start_idx}-{end_idx}") as pbar:
